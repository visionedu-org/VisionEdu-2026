...
import {
  isStudentNavActive,
  STUDENT_NAV_LINKS,
} from "@/features/student/components/student-nav-links";
  ...
  return (
    <header className="sticky top-0 z-40 bg-background/95 px-4 py-2 backdrop-blur-md md:hidden">
      <div className="flex items-center justify-between gap-3">
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          >
            <DialogHeader className="border-b border-border/60 px-5 py-5">
              <DialogTitle>Menu do aluno</DialogTitle>
            </DialogHeader>
            <nav
              aria-label="Navegação do aluno"
              className="flex flex-col gap-1 p-4"
            >
              {STUDENT_NAV_LINKS.map(({ href, label }) => {
                const active = isStudentNavActive(pathname, href);
                return (
                  <Link
                    key={href}
                    href={href}
        ...
        <UserProfileMenu roleLabel="Aluno" />
      </div>
    </header>
  );
}
